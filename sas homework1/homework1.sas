...
/* hwk1 */
	...
	proc freq data = hwk1.dataset3;
		tables dead30 dead1095 / out = hwk1_1;
	run;
	...
	/* hwk1-2 risk if day(10, 100, by = 10) */
		...
		/* calculate risk */ 
		proc freq data = hwk1_2;
			tables dead_date / out = hwk1_2_risk outcum;
		run;

		data hwk1.hwk1_2_risk;
			set work.hwk1_2_risk;
		run;
...
/* hwk3 */
	...
	proc lifetest data = hwk1.dataset3 plots = (s) method = KM;
	...
	/* hwk3-2 cumulative risk of competing risk */
		/* grouping */
		data hwk1.hwk3_2_competing_risk;
			set hwk1.dataset3;
		...
		/* just to check */
		proc print data = hwk1.hwk3_2_competing_risk;
			var deadfinal MRSA_relateddeath deadstatus;
		run;

		/* plotting cumulative risk curve */ 
		proc lifetest data = hwk1.hwk3_2_competing_risk plots = CIF outcif = cr_mrsa	; 
			time dayfinal * deadfinal(0) / failcode = 1;
			strata MRSA_relateddeath;
		run;
